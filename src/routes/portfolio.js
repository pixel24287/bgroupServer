...
router.post('/', upload.array("files", 3), async (req, res) => {
    ...
    try {
        ...
        if (req.files.length > 0) {
            ...
            const ins1 = await conn.query('INSERT INTO `portfolio` SET ?', [newPortfolio]);
            await conn.commit();
...
router.get('/total_page', async (req, res) => {
    ...
    try {
        await conn.beginTransaction();
        const ins1 = await conn.query('SELECT COUNT(*) as count FROM `portfolio`')
        await conn.commit();
...
router.get('/list/:id', async (req, res) => {
    ...
    try {
        ...
        const ins1 = await conn.query('SELECT * FROM `portfolio` LIMIT ?, 12;', [pageNumber]);
        await conn.commit();
